...
class FeedViewController: UIViewController {
    ...
    private func queryPosts(completion: (() -> Void)? = nil) {
        ...
        let query = Post.query()
            ...
            .where("createdAt" >= yesterdayDate) // <- Only include results 24hrs ago
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        // 1. Check if this is our detail segue
        if segue.identifier == "DetailSegue",
           // 2. Get the cell that was tapped
           let cell = sender as? UITableViewCell,
           // 3. Find the index of that cell
           let indexPath = tableView.indexPath(for: cell),
           // 4. Get the destination controller
           let detailVC = segue.destination as? PostDetailsViewController {
            
            // 5. Pass the specific post to the detail screen
